...
LABEL maintainer="Opscale <development@opscale.co>"
LABEL org.opencontainers.image.title="Laravel Octane optimized Dockerfile"
...
COPY --link --chown=${WWWUSER}:${WWWUSER} docker/octane/Swoole/supervisord.swoole.conf /etc/supervisor/conf.d/
COPY --link --chown=${WWWUSER}:${WWWUSER} docker/supervisord.*.conf /etc/supervisor/conf.d/
...
COPY --link --chown=${WWWUSER}:${WWWUSER} composer.json composer.lock auth.json ./
